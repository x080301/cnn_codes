...
training_file = current_file_dir+'/data/train.p' #'../data/train.p'
validation_file= current_file_dir+'/data/valid.p' #'../data/valid.p' 
testing_file = current_file_dir+'/data/test.p' #'../data/test.p'
...
with open(current_file_dir+'/data/signnames.csv','r') as sign_name:
...
signs_ids = pd.read_csv(current_file_dir+'/data/signnames.csv')
signs_ids.head(5)
...
with tf.Session() as sess:
    ...
    saver.save(sess, current_file_dir+'/data/lenet')
    print("Model saved")
...
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    saver1 = tf.train.import_meta_graph(current_file_dir+'/data/lenet.meta')
    saver1.restore(sess, current_file_dir+'/data/lenet')
    trai_accuracy = evaluate(X_train_p, y_train)
    print("Train Set Accuracy = {:.3f}".format(trai_accuracy))

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    saver2 = tf.train.import_meta_graph(current_file_dir+'/data/lenet.meta')
    saver2.restore(sess, current_file_dir+'/data/lenet')
